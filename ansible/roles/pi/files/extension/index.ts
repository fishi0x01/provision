 ...
 * isolated to the VM.
 *
 * Vendored from pi's examples/extensions/gondolin and patched here to add an
 * egress allowlist and custom guest image selection - upstream calls
 * VM.create() with no httpHooks, which leaves the VM with no network at all.
 ...
 *   - QEMU installed (provisioned by the gondolin ansible role)
 */
...
interface GondolinConfig {
	/** Hostnames the guest may reach over HTTP/HTTPS. Supports `*` wildcards. */
	allowedHosts: string[];
	/** Hosts that additionally bypass the internal-IP block (e.g. a LAN registry). */
	allowedInternalHosts: string[];
	/** Asset directory of a custom guest image, or null for the builtin default. */
	imagePath: string | null;
	/** Grow the writable root disk before boot, e.g. "4G". Needs resize2fs in the image. */
	rootfsSize: string | null;
}
...
export default function (pi: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		// Without httpHooks the guest has no egress at all, so package installs
		// and git fetches inside the VM fail. The allowlist is enforced host-side
		// against the HTTP Host header, not by whatever the guest resolved.
		...
		const created = await VM.create({
			...
			httpHooks,
			env,
			...(config.imagePath ? { sandbox: { imagePath: config.imagePath } } : {}),
